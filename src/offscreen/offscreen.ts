...
function subscribeProjectChannel(projectId: string) {
  ...
  const channelName = `entry-sync:${projectId}`;
...
function handleSend(msg: { kind: string; projectId: string; name: string; value?: unknown; operation?: string; args?: unknown[]; timestamp: number; _senderTabId?: number }) {
  let channel = channels.get(msg.projectId);
  if (!channel) {
    console.log('[EntrySync:Offscreen] No channel yet, subscribing...');
    subscribeProjectChannel(msg.projectId);
    channel = channels.get(msg.projectId);
  }
  if (!channel) {
